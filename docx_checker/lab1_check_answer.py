...
def lab_1_check_answer(student_path, correct_path):
    ...
    try:
        ...
        document_student = Document(document_path)
        ...
        if get_footnotes(document_student).sort() == get_footnotes(document_correct).sort():
         ...
         if get_docement_headers(document_student, header_style_name).sort() == get_docement_headers(document_correct, header_style_name).sort():
        ...
        if get_document_margins(document_student) == get_document_margins(document_correct):
            response_message["document_header"]["message"] = "Отступы применены верно"
            response_message["document_header"]["status"] = True
        else:
            response_message["document_header"]["message"] = "Отступы применены неверно"
            response_message["document_header"]["status"] = False
